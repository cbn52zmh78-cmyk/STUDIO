# NadiaOkoroMag-001 — Nadia Okoro

**Lane:** MAGAZINE  
**Status:** LOCKED  
**Content rating:** SFW  
**Synthetic:** Yes — no real-person likeness  
**AI disclosure required:** Yes

## Identity
| Field | Value |
|-------|-------|
| Actor ID | `NadiaOkoroMag-001` |
| Stage name | Nadia Okoro |
| Talent ID | `mag_nadia_okoro` |
| Age (locked, **21+**) | **28** |
| Gender | female |
| Ethnicity | Nigerian-French |
| Region | editorial_global |
| Roster source | magazine_editorial |

## Reference
- **Primary plate:** `PENDING — generate casting_turnaround_v1.jpg`
- **Roster path:** `STUDIO/MAGAZINE/Editorial/Models/Nadia Okoro`

## Appearance lock (verbatim — reuse every shot)
```
photorealistic high-fidelity 16:9 magazine editorial photograph, single 28-year-old Nigerian-French woman named Nadia Okoro, radiant dark-skinned supermodel with intricate braids and gold threads, high cheekbones, flawless glowing skin, powerful striking presence, perfect symmetry, wearing bold gold and saffron African-inspired avant-garde wrap dress with sculptural sleeves, elegant confident pose with intense captivating gaze to camera, dramatic cinematic lighting with soft shadows and high contrast highlights on fabric and flawless skin, ultra-detailed textures, sharp focus, Vogue-level professional fashion photography, expensive avant-garde haute couture aesthetic, natural physics dress drape and movement, commercial-ready high-end magazine cover quality
```

## Voice spec
```json
{
  "register": "derived_from_notes",
  "notes": "Editorial \u2014 voice TBD.",
  "performance_style": null,
  "prompt_suffix": "Editorial \u2014 voice TBD.",
  "gender": "female"
}
```

## Tags
- **Persona:** radiant dark-skinned supermodel with intricate braids and gold threads
- **Genre:** editorial_hero, fashion, haute_couture, magazine_editorial, runway

## Guardrails
- Synthetic-only; do not reference or generate real-person/celebrity likeness.
- Lead every generation prompt with exact age: `28-year-old` (**minimum 21+**).
- SFW wardrobe only — suggestive-clothed acceptable; no nude/topless/explicit.
- AI disclosure required on all shipped content.

---
*STUDIO Casting Bible lock card — do not paraphrase appearance_lock_verbatim in production prompts.*
